...
Indicator.prototype = {
    ...
    _init: function() {
        ...
        this._control.connect('stream-added', Lang.bind(this, this._maybeAddOutput));
        this._control.connect('stream-removed', Lang.bind(this, this._maybeRemoveOutput));
    ...
    _readOutput: function() {
        ...
        if (this._output) {
            ...
            if (this._outputId != this._output.id) {
                if (this._outputId in this._outputMenus)
                    this._outputMenus[this._outputId].setShowDot(false);
                this._outputId = this._output.id;
                if (this._outputId in this._outputMenus)
                    this._outputMenus[this._outputId].setShowDot(true);
            }
        } else {
            this._outputSlider.setValue(0);
            this.setIcon('audio-volume-muted-symbolic');
            if (this._outputId >= 0 && this._outputId in this._outputMenus) {
                this._outputMenus[this._outputId].setShowDot(false);
                this._outputId = -1;
            }
        }
    ...
    _maybeAddOutput: function(control, id) {
        ...
        if (stream instanceof Gvc.MixerSink) {
            ...
            let outputOffset = 2;
            this.menu.addMenuItem(menu, outputOffset + this._outputCount);
            ...
            this._outputCount++;
            if (this._output && stream.id == this._output.id)
